  ...
  return (
    <div className="bg-white rounded-md shadow-sm border p-6">
      <div className="flex justify-between items-center mb-6 pb-4 border-b">
        <div>
          <h2 className="text-xl font-bold">{isNew ? "New Sales Order" : `SO-${order.id.slice(-8).toUpperCase()}`}</h2>
          {!isNew && order.invoice && (
            <div className="text-sm mt-1 text-blue-600 font-medium">
              Linked Invoice: INV/{order.invoice.id.slice(-8).toUpperCase()} ({order.invoice.status})
            </div>
          )}
        </div>
